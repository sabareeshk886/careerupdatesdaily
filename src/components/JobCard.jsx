    ...
    return (
        ...
        >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div>
                    <h3 style={{ margin: '0 0 0.25rem 0', fontSize: '1.125rem', fontWeight: '600', color: 'var(--text-primary)' }}>{job.title}</h3>
                    <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.95rem' }}>{job.company}</p>
                </div>
                {job.type && (
                    <span className="tag" style={{ background: '#F7F7F7', border: '1px solid #EAEAEA', color: 'var(--text-secondary)' }}>
                        {job.type}
                    </span>
                )}
            </div>
